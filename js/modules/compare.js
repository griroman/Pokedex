...
var compare = (function($) {
    ...
    // Добавление покемона к сравнению
    function _onClickAddToCompare(e) {
        ...
        // Проверяем, нет ли этого покемона уже в куках
        if (comparedGoodsArr.indexOf(goodId) === -1) {
            // Добавляем нового покемона в массив сравниваемых
            ...
            updateCompareTab();
            $button.html('Покемон сравнивается');
        } else {
            alert('Этот покемон уже есть в списке сравниваемых');
        }
    }
    ...
    // Удаление покемона из списка сравниваемых
    function _onClickCompareRemove(e) {
        ...
        // Удаляем куки, если исключили всех покемонов
        if (!newGoodsStr) {
    ...
    // Обновление количества сравниваемых покемонов во вкладке
    function updateCompareTab() {
        ...
        // Обновляем метку с количеством покемонов во вкладке compare
    ...
    function _getBaseProps(goods) {
        ...
        return _.map(baseProps, function(item) {
            ...
            // Одинаковые ли значения у ВСЕХ покемонов
    ...
    function _renderCompareTable(response) {
        ...
        // Рендерим покемонов в шапке таблицы
        ui.$compareTable.find('thead tr').append(tpl.header({
            goods: goods
        }));

        // Рендерим свойства покемонов в таблице
